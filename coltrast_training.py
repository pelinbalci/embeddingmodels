...
class ColTrastDataset(Dataset):
    ...
    def _process_dataset(self) -> List[Dict]:
        ...
        for project in self.data['projects']:
            ...
            for qa in qa_pairs:
                ...
                best_chunk = self._find_best_chunk(question + ' ' + answer, chunks, answer)
                ...
                negative_chunks = self._get_negative_chunks(project_id, question + ' ' + answer)
    ...
    def _find_best_chunk(self, query_text: str, chunks: List[Dict], answer_text: str) -> Dict:
        """Find the most relevant chunk for a query"""
        if not chunks:
            return {"text": "", "chunk_id": 0}

        # Use sentence-transformers for semantic similarity
        query_emb = self.chunker.encoder.encode([query_text])
        chunk_texts = [c['text'] for c in chunks]
        chunk_embs = self.chunker.encoder.encode(chunk_texts)

        similarities = cosine_similarity(query_emb, chunk_embs)[0]
        max_idx = int(np.argmax(similarities))
        max_score = similarities[max_idx]

        if max_score > 0.6:  # Threshold — tune based on your data
            return chunks[max_idx] | {'source': 'readme'}
        else:
            return {"text": answer_text, "chunk_id": -1, "source": "qa_only"}

    def _get_negative_chunks(self, current_project_id: str, qa_text: str, top_k: int = 2) -> List[Dict]:
        """
        Return top-k semantically similar but incorrect chunks as hard negatives.
        """
        candidate_chunks = []

        for project in self.data['projects']:
            if project['project_id'] != current_project_id:
                other_chunks = self.chunker.chunk_text(project['readme_content'])
                candidate_chunks.extend(other_chunks)

        # Embed QA text and all candidate chunks
        qa_embedding = self.chunker.encoder.encode([qa_text])
        chunk_texts = [chunk['text'] for chunk in candidate_chunks]
        chunk_embeddings = self.chunker.encoder.encode(chunk_texts)

        # Compute similarities
        similarities = cosine_similarity(qa_embedding, chunk_embeddings)[0]

        # Get top-k most similar (hard negatives)
        hard_negatives = np.argsort(similarities)[-top_k:][::-1]

        return [candidate_chunks[i] for i in hard_negatives]
...
class ColTrastModel(nn.Module):
    ...
    def encode_text(self, input_ids, attention_mask):
        ...
        # Mean pooling
        token_embeddings = outputs.last_hidden_state #8,512,384
        attention_mask_expanded = attention_mask.unsqueeze(-1).expand(token_embeddings.size()).float() #8,512 to 8,512,384

        pooled_embedding = torch.sum(token_embeddings * attention_mask_expanded, 1) / torch.clamp(
            attention_mask_expanded.sum(1), min=1e-9) # 8,384

        # Apply projection head
        projected_embedding = self.projection_head(pooled_embedding) # 8,384
    ...
    def late_interaction_similarity(self, question_tokens, chunk_tokens, q_mask, c_mask):
        ...
        mask = (q_mask_expanded & c_mask_expanded).bool()
        ...
        max_similarities = max_similarities.masked_fill(~q_mask.bool(), 0)
